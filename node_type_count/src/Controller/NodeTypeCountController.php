...
class NodeTypeCountController extends ControllerBase {
   ...
   *   Returns the count of node published or unpublished in the Content Type.
   */
  public function hello($imgageid) {
  ...
  /*public function imgReplace() {
    ...
    $fid = $_GET['imageid'];
    $replace_path = "/sites/default/files/";
    //$get_filename = get_filename_by_fid($fid);
    $query = \Drupal::database()->select('file_managed', 'nfd');
        $query->fields('nfd', ['filename']);
        $query->condition('nfd.fid', $fid);
        $query->range(0, 1);
        $filename_by_fid = $query->execute()->fetchField();
    $replace_path = $replace_path . $filename_by_fid;
    $response = new RedirectResponse($replace_path, 301);
    $response->send();
 ...
 public function imgReplace() {
    $host = \Drupal::request()->getHost();
    $fid = SafeMarkup::checkPlain($_GET['imageid']);
    $replace_path = "/sites/default/files/FileCenter/images/";
    //Query the table for the path
    $query = \Drupal::database()->select('sf_map_visionimage', 'nfd');
        $query->fields('nfd', ['file_name']);
        $query->condition('nfd.id', $fid);
        $query->range(0, 1);
    $filename_by_fid = $query->execute()->fetchField();
    if($filename_by_fid){
      $replace_path = $replace_path .$fid."-". $filename_by_fid;
      $response = new RedirectResponse($replace_path, 301);
      $response->send();
      exit();
    }
